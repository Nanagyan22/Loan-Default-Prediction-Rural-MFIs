...
def main():
    ...
    <div class="main-header">
        ...
        <h3>Machine Learning Credit Scoring System</h3>
        <p>Rural Microfinance Risk Assessment Platform</p>
    ...
    with st.sidebar:
        ...
        if len(available_models) > 0:
            selected_model = available_models[0]  # Always use first available model (RF)
    ...
    <div style="text-align: center; padding: 20px 0; color: #666; font-size: 14px;">
        🧠 Applied Machine Learning Project · University of Ghana (UoG) · July 2025
    </div>
    """, unsafe_allow_html=True)
...
def single_assessment_interface():
    ...
    with st.form("borrower_assessment"):
        ...
        with col3:
            ...
            loan_to_income_ratio = st.number_input("Loan-to-Income Ratio", min_value=0.0, max_value=20.0, value=3.3, step=0.1,
                                                   help="Ratio of loan amount to monthly income (auto-calculated if 0)")
            debt_to_income_ratio = st.number_input("Debt-to-Income Ratio", min_value=0.0, max_value=20.0, value=1.0, step=0.1,
                    ...
                    try:
                        ...
                        try:
                            batch_data = {
                                ...
                                'high_risk_count': len(results_df[results_df['risk_band'].isin(['High Risk', 'Very High Risk'])]),
                                'approval_rate': len(results_df[results_df['default_probability'] < 0.3]) / len(results_df) * 100,
                        ...
                        with stat_col2:
                            ...
                            **Interpretation Guide:**
                            - **Mean**: Average default risk across all applicants
                        ...
                        display_cols = ['applicant_id', 'default_probability', 'risk_band', 'recommendation', 'Decision Reason']
    ...
    try:
        ...
        if alerts:
            for alert in alerts:
                ...
                st.warning(f"{severity_color.get(alert['severity'], '⚪')} **{alert['group']}** - {alert['metric']}: {alert['value']} - {alert['message']}")
        else:
...
def what_if_simulator_interface():
    ...
    with col2:
        ...
        mod_gender = st.selectbox("Gender", ["Male", "Female"], index=["Male", "Female"].index(base_gender), key="mod_gender")
...
def performance_tracking_interface():
    ...
    with st.expander("🔬 Demo: Add More Simulated Outcomes"):
        st.info("In production, actual outcomes would be updated as loans mature. For demonstration, you can add more simulated outcomes.")
...
def analytics_dashboard():
    ...
    if st.button("🔄 Generate Analytics", use_container_width=True):
        ...
        with col1:
            st.subheader("🗺️ Risk Distribution by Region")
            fig = px.bar(
                risk_df,
                x='Region',
                y='Count',
...
def model_information_interface():
    ...
    **⚠️ Important:** If you encounter any discrepancies in model predictions or 
    ...
    'UGBS-Case_Loan (2)_1760030105965.ipynb' for detailed implementation and validation steps.
    """)
